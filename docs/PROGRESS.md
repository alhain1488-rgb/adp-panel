...
## Пост-10 — Stars: суммы под тарифы + возврат звёзд ✅ (v0.9.12.0)

Доработка Telegram Stars по итогам аудита существующего флоу. Найдены и закрыты два пробела:
пресеты пополнения не бились с ценами тарифов (годовой тариф одним пополнением купить было
**нельзя** — максимальный пресет 1000 ⭐ = 1300 ₽ против цены 2000 ₽), и возврат из SPEC §14 был
заявлен как задел, но не реализован.

- **Суммы под тарифы.** `sendStarAmounts` теперь берёт клиента и его баланс: сверху — по кнопке на
  каждый тариф, посчитанной как `ceil((цена − баланс) / курс)` (новый `starsFor`, округление вверх —
  нехватка одной звезды оставила бы клиента без возможности купить тариф, ради которого он нажал
  кнопку); ниже — прежние круглые пресеты. Тарифы, которые баланс уже покрывает, не показываются;
  если покрыты все — подсказка «нажмите Купить подписку».
- **Возврат звёзд.** Миграция `0007` (аддитивная): колонка `refunded_at` на `billing_transactions`.
  `store.RefundStarTopup` — атомарно: перепроверка внутри транзакции, списание с кошелька, штамп на
  исходной строке, компенсирующая строка `refund` **с пустым `charge_id`** (частичный уникальный
  индекс остаётся свободен для идемпотентного зачисления). Новые ошибки `ErrNotRefundable` /
  `ErrAlreadyRefunded`.
- **Порядок операций.** `billing.RefundStars` валидирует всё → зовёт Telegram → и только потом двигает
  леджер. Обратный порядок мог бы списать деньги у клиента, которому звёзды не вернулись. Транспорт —
  интерфейс `StarRefunder` (`SetStarRefunder` в `main.go`), чтобы не получить цикл импорта, т.к.
  `backup` уже импортирует `billing`. Проверка транспорта стоит **последней**, иначе несуществующая
  операция отвечала бы 503 вместо 404 (поймано тестом).
- **Потраченный баланс блокирует возврат** (решение Заказчика): если на кошельке меньше, чем
  зачислялось, — 409, баланс никогда не уходит в минус, разбор такого случая за админом.
- HTTP: `POST /api/clients/{id}/billing/refund` (409 на неподходящую/повторную/непокрытую операцию,
  502 на отказ Telegram), запись в audit log; `openapi.yaml` дополнен. Фронт: `useClientRefund`,
  кнопка «Вернуть» в истории + диалог подтверждения с точными суммами, бейдж «Возвращено»;
  `refundable`/`refunded` считает бэкенд, фронт их только отображает. Моки повторяют контракт
  (сид-пополнение звёздами, чтобы флоу был обозрим).

**Проверено:** backend `go build/vet/gofmt/test` — 17 пакетов зелёные, включая новые
`TestRefundStarTopup`, `TestRefundStarTopup_SpentBalanceIsBlocked`, `TestRefundStars`,
`TestRefundStars_TelegramFailureLeavesLedgerAlone`, `TestRefundStars_GuardsSpentBalanceAndUnlinkedClients`,
`TestBilling_RefundStarsEndpoint`, `TestStarsFor` + property-тест `TestStarsForCoversTheNeed`;
frontend `tsc/lint/test/build`. Превью на моках: возврат 100 ⭐ → баланс 130 → 0 ₽, в истории строка
−130 ₽, исходное пополнение помечено «Возвращено», кнопка исчезла, консоль чистая.

**Не сделано:** живой тест на проде (нужны действия Заказчика в Telegram) — `golangci-lint` в среде
не установлен, прогнаны `go vet` + `gofmt`.

## Пост-10 — Пожизненный бесплатный доступ (billing_exempt) ✅ (v0.9.13.0)

Перед включением биллинга на проде текущим клиентам закреплён бесплатный доступ навсегда.

Де-факто они и так не отключались: автоприостановка бьёт только по `billing_managed = 1`, а этот
флаг ставится при первой покупке/выдаче. Но гарантия держалась ровно до первой транзакции — купив
что-нибудь однажды, клиент становился managed навсегда. Поэтому сделан **явный** флаг.

- Миграция `0008` (аддитивная): колонка `billing_exempt` на `clients` + `UPDATE clients SET
  billing_exempt = 1` — все, кто существовал на момент наката, получают пожизненный доступ. Созданные
  позже — по умолчанию `0`.
- `SuspendExpiredManaged`: в WHERE добавлено `billing_exempt = 0`. Это единственное место, где
  движок кого-либо выключает, поэтому одной строки достаточно.
- `billing.SetExempt` + `ClientBilling.Exempt`; `POST /api/clients/{id}/billing/exempt` с записью
  в audit log; `openapi.yaml` дополнен.
- **Флаг не включает выключенного клиента.** Освобождение — обещание про будущее, а не отмена
  ручного тумблера админа; иначе снятие галочки «Выключен» происходило бы молча. В подсказке под
  тумблером это сказано прямо.
- Фронт: тумблер + зелёный бейдж «Пожизненно бесплатно» в карточке клиента, `useClientExempt`, моки.

**Проверено:** backend `go build/vet/gofmt/test` — все пакеты зелёные, включая
`TestReconcile_NeverSuspendsExemptClients` (платящий истёкший клиент выключается, освобождённый — нет;
после снятия флага выключается и он) и `TestSetExempt_DoesNotReenableDisabledClient`;
frontend `tsc/lint/test/build`. Превью на моках: тумблер → бейдж «Пожизненно бесплатно», тост
«Применено», консоль чистая.

## Пост-10 — Деплой на прод + вкладка «Оплата» всегда видна ✅ (v0.9.13.1)

Развёрнуто на VPS (Ubuntu, 1 CPU / 1 ГБ RAM + 2 ГБ swap, домен `adpanel.work.gd`). Прод стоял на
**0.9.10.0** — биллинг туда не деплоился ни разу, применены были только миграции 1–5. Этим накатом
впервые появились кошельки, возвраты и пожизненный доступ: миграции `0006`, `0007`, `0008`
применились, у всех **6 существующих клиентов** проставлен `billing_exempt = 1`.

**Найдено на живом деплое:**
- **Версия приложения жила в двух местах** — `frontend/src/version.ts` и `const version` в
  `backend/cmd/panel/main.go`. Правило в CLAUDE.md называло только первое, поэтому свежесобранная
  панель отдавала в `/healthz` 0.9.11.0, обслуживая 0.9.13.0. Обе строки синхронизированы, правило
  в CLAUDE.md дополнено вторым файлом — расхождение больше ничем не ловится.
- **Вкладка «Оплата» в карточке клиента была скрыта при выключенном биллинге** и админ её не находил.
  Теперь она видна всегда: с выключенной оплатой показывается баннер «Оплата выключена — включите в
  Настройки → Оплата» (ключ `clientDetail.billing.disabledHint` был написан ещё в прошлой итерации,
  но нигде не использовался), а баланс, история и тумблер пожизненного доступа остаются доступны.

**Порядок деплоя (воспроизводимый):** бэкап БД через `sqlite3 .backup` в `/root/adp-backups/` +
копия `.env` → `docker builder prune -af` (диск на 83%, кэш отдал 1.19 ГБ) → `git archive HEAD |
ssh … tar -x` в `/opt/adp-panel` (без push, `.env` не перезаписывается) → `docker compose up -d --build`.

**Проверено:** контейнеры `Up`, backend `healthy`, `https://adpanel.work.gd/healthz` → 200,
`schema_migrations` = 1–8, все 6 клиентов `enabled=1, billing_exempt=1`. Backend `build/vet/gofmt/test`,
frontend `tsc/lint/build` — зелёные.

**Не сделано:** живой тест оплаты звёздами — биллинг на проде ещё выключен (`billing_enabled` в
настройках не выставлен), включение и сама оплата за Заказчиком.

## Пост-10 — Самостоятельная покупка подписки через бота ✅ (v0.9.14.0)

Раньше бот обслуживал только клиентов, созданных админом (`/start <токен>`). Теперь посторонний
может купить доступ сам — за тумблером `billing_selfsignup_enabled` (по умолчанию **выключен**,
чтобы включение биллинга не открывало продажу молча). Решения Заказчика — `QUESTIONS.md` Q7.

- `clients.CreateSelfSignup` — создаёт клиента **выключенным, без подписки**, но с доступом ко всем
  включённым инбаундам (`store.ListAllEnabledInboundIDs`) и сразу привязывает Telegram-чат.
  До оплаты клиент не попадает ни в один конфиг движка (sync берёт `enabled = 1`), поэтому
  доступа нет; покупка включает его и триггерит пересинхронизацию.
- Бот: `payingClient` заводит клиента на первом шаге к оплате (кнопки «Купить»/«Пополнить», выбор
  суммы) — не на `/start`, чтобы простое открытие бота не мусорило в списке клиентов. Незнакомцу на
  `/start` показывается приветствие с тарифами (`sendWelcome`), без создания записи. Привязка к
  чату делает регистрацию идемпотентной: второй заход находит существующего клиента.
- Транспорт регистрации — интерфейс `backup.Signup`, реализуемый `*clients.Service` и подключаемый
  в `main.go` (тот же приём, что и `StarRefunder`: `backup` уже импортирует `billing`).
- Имя клиента — `tg:@handle`, либо `tg:<chat_id>` если хэндла нет. Уникальность имён не требуется.
- Настройки → Оплата: тумблер «Самостоятельная покупка в боте» (неактивен, пока не включён приём
  оплаты). `openapi.yaml` и моки дополнены.

**Проверено:** backend `go build/vet/gofmt/test` — все пакеты зелёные, включая `TestCreateSelfSignup`
(клиент выключен, не managed, не exempt, выданы ровно включённые инбаунды, отсутствует в списке для
конфига ноды, находится по chat id) и `TestSignupName`; frontend `lint/test/build`; превью на моках —
тумблер рендерится в нужной карточке.

**Поймано typecheck-ом:** правка настроек скриптом задела ещё две карточки (почта и Telegram-бэкап) —
`tsc -b` показал это сразу, `tsc --noEmit` промолчал. Проверять фронт через `npm run build`.

## Пост-10 — Доводка самозаписи по живому пути ✅ (v0.9.14.1)

Перед включением самозаписи на проде прошёл сквозной путь незнакомца и нашёл два места, где он
упирался в тупик:

- **Повторный `/start` отдавал нерабочий конфиг.** После регистрации чат уже привязан, поэтому
  ветка «клиент найден» слала подписку — но неоплаченный клиент выключен, а значит отсутствует во
  всех конфигах движков. Человек получал ссылки, которые не подключаются. Теперь при включённом
  биллинге выключенному клиенту показываются тарифы. Это же чинит и случай истёкшей подписки:
  вместо мёртвых ссылок предлагается продлить.
- **После регистрации не было меню.** Reply-клавиатура отправлялась только вместе с конфигом, то
  есть уже после оплаты; бросивший на полпути не мог вернуться в поток иначе как заново набрав
  `/start`. Теперь клавиатура уходит вместе с приветствием при создании аккаунта.

**Проверено:** backend `go build/vet/gofmt/test` — все пакеты зелёные.

## Пост-10 — Баланс звёзд бота в панели ✅ (v0.9.15.0)

Оплаченные звёзды копятся **на боте** и хранятся у Telegram — ни панель, ни личный баланс админа их
не видят. Раньше посмотреть их можно было только через @BotFather. Теперь Настройки → Оплата
показывают баланс и последние операции.

- `internal/backup/telegram_stars.go`: `StarBalance` — два вызова Bot API (`getMyStarBalance`,
  `getStarTransactions`) плюс общий `callBotAPI`, декодирующий конверт `{ok, result, description}`
  (существующий `tgDo` проверял только `ok` и результат не возвращал). Направление операции
  определяется по тому, какая сторона заполнена: `source` → приход, `receiver` → расход.
  Без токена бота возвращается `configured: false`, а не нули — это разные вещи.
- `GET /api/billing/stars` (только админ) + карточка «Баланс звёзд бота» в Настройки → Оплата.
- **Данные читаются живьём из Telegram, а не из нашего леджера** — они отвечают на разные вопросы:
  леджер хранит, сколько рублей начислено клиентам, а этот баланс — что реально лежит у Telegram.
  Курс ⭐→₽ из настроек бухгалтерский; сколько выйдет на выводе, зависит от Fragment и курса TON.
- В подсказке под балансом зафиксированы условия вывода, чтобы не искать их заново: заморозка
  **21 день** (это же окно возврата), минимум **1000 ⭐**, вывод через **Fragment в TON**, нужен KYC.

**Проверено:** backend `go build/vet/gofmt/test` — зелёные; frontend `lint/test/build`; превью на
моках — баланс и три операции с верными знаками рендерятся, консоль чистая.

## Пост-10 — Оплата картой/СБП через Tribute ✅ (v0.10.0.0)

Второй способ оплаты рядом со звёздами. Решения и разведка по API — `QUESTIONS.md` Q8.

**Что выяснилось при разведке (важнее самого кода):**
- Магазин (Shop API) **создать самому нельзя**: в официальной OpenAPI-спеке Tribute
  (`https://tribute.tg/api/v1/openapi/en`) нет ни `/shops`, ни `/shop/orders`, ни `shopOrder*`-вебхуков,
  а метода создания магазина нет нигде. `/api/v1/shops` с рабочим ключом отдаёт `[]`. Значит это
  отдельный продукт, подключаемый вручную поддержкой. Поэтому пошли через **цифровые товары** —
  они самообслуживаемые и полностью описаны в спеке.
- Вебхук `newDigitalProduct` несёт **обязательные** `purchase_id` и `telegram_user_id` — этого
  достаточно и для идемпотентности, и для сопоставления с клиентом.

**Модель.** Товар Tribute имеет фиксированную цену и покупается напрямую: кошелёк не участвует,
покупка сразу выдаёт подписку. Панель хранит только соответствие `product_id → дней`, поэтому
Tribute может продавать периоды, которых нет в тарифах панели (шестимесячный), не меняя модель
тарифов. Цены сознательно расходятся со звёздными (решение Заказчика): звёзды дороже, карта дешевле.

- Новый пакет `internal/tribute`: настройки (тумблер, API-ключ шифрованный at-rest, список товаров),
  проверка HMAC-SHA256 подписи `trbt-signature` по **сырому** телу и обработка вебхука.
- `store.GrantSubscription` получил проверку `charge_id` — ту же, что была у `CreditWallet`.
  Новой таблицы не понадобилось: частичный уникальный индекс на `charge_id` уже даёт идемпотентность,
  ключ — `tribute:<purchase_id>`.
- `billing.GrantPaid` — подписка, оплаченная мимо кошелька: сумма пишется в леджер для учёта, но
  **на баланс не зачисляется**, иначе её можно было бы потратить второй раз.
- `POST /api/tribute/webhook` — четвёртый публичный маршрут. Тело читается вербатим (перекодирование
  сломало бы подпись), rate-limit 60/мин. Всё, что применить нельзя (неизвестный товар, незнакомый
  плательщик, чужой тип события), отвечает **200** — Tribute ретраит сутки, и повторять то, что мы
  всё равно не примем, бессмысленно. Настоящие сбои отдают 500, чтобы ретрай был полезен.
- Бот: кнопки 💳 с тарифами Tribute встали рядом с тарифами кошелька на экране «Купить подписку» —
  они выдают подписку сразу, поэтому там, а не под «Пополнить».
- Настройки → Оплата: карточка «Карта и СБП через Tribute».

**Проверено:** backend `go build/vet/gofmt/test` — все пакеты зелёные, включая пять тестов
`internal/tribute` (валидная подпись выдаёт подписку с верным charge_id; подделанная, пустая и
мусорная подписи отклоняются; **подменённое после подписи тело** — попытка сменить период или
плательщика — отклоняется; неизвестный товар/плательщик/чужое событие игнорируются; выключенный и
ненастроенный Tribute ничего не пропускает; ключ переживает сохранение с пустым полем) и
`TestGrantPaid_IdempotentByChargeID` (повтор вебхука не продлевает подписку и не пишет вторую строку).
Frontend `lint/test/build`.

**Не проверено вживую и почему:** товары Заказчика имеют `status: "new"` (спека знает только
`pending/approved/rejected`) и `type: "custom"`, а вебхук называется `newDigitalProduct` и описан
для цифровых товаров. Сработает ли он для `custom` — из документации не следует. СБП у товаров тоже
пока нет (`acceptCards`/`acceptWalletPay` есть, флага СБП нет). Всё три пункта — вопросы к поддержке
Tribute, на устройство кода они не влияют.
