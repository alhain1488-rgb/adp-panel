...
> **СОСТОЯНИЕ:** **Все фазы 0–9 завершены.** Панель развёрнута на VPS человека (Ubuntu 22.04),
> работает по HTTPS с доверенным сертификатом Let's Encrypt (через sslip.io-хостнейм, домена нет);
> движки провижинятся, клиенты/подписки/VPN проверены рабочим трафиком; UI отревьюен человеком.
> Проект запушен в приватный GitHub-репозиторий, ветка `build/mvp`.
> Версия — `frontend/src/version.ts` (сейчас 0.8.1.3; бампать при правках). Docker — через **Colima**
> локально; на VPS — `docker compose up` (backend + Caddy). Данные деплоя (IP/SSH) — только в сессии,
> в репозиторий не попали (проверено сканом всей истории). Дальше — по запросам человека.
