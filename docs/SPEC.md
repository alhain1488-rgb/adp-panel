...
**Самостоятельная покупка (self-signup).** Отдельный тумблер `billing_selfsignup_enabled`
(по умолчанию выключен). С ним любой, кто открыл бота, видит тарифы; на первом шаге к оплате
панель заводит клиента: **выключенного, без подписки**, но уже с доступом ко всем включённым
инбаундам. До оплаты он не попадает ни в один конфиг движка, поэтому доступа нет. Покупка
включает клиента и запускает пересинхронизацию нод. Клиент привязывается к Telegram-чату сразу,
поэтому повторный заход не плодит дубли. Решения — `QUESTIONS.md` Q7.
